...
function eventTitle(item) {
  return item.event_title || item.event?.title || `Request #${item.id}`;
}

function eventDescription(item) {
  return item.event_description || item.event?.description || 'No description provided.';
}

function eventVenue(item) {
  return item.event_location || item.event?.venue || 'Venue not set';
}
            ...
            {items.map((item) => (
              <article key={item.id} className="p-6">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-slate-950">{eventTitle(item)}</h3>
                    <p className="mt-2 max-w-3xl text-slate-500">{eventDescription(item)}</p>
                    <p className="mt-2 text-sm font-medium text-slate-500">{eventVenue(item)}</p>
                  </div>
                  <div className="flex gap-3">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 font-bold text-white hover:bg-emerald-700 disabled:bg-slate-300"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 font-bold text-white hover:bg-red-700 disabled:bg-slate-300"
                    >
