        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`rounded-full px-6 py-3 text-lg font-bold ${
              active === tab ? 'bg-[#073f73] text-white' : 'bg-white text-slate-600'
            }`}
          >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {visible.length ? (
                visible.map((item) => (
                  <tr key={item.id}>
                    <td className="px-6 py-5 font-semibold">{eventTitle(item)}</td>
                ...
                <tr>
                  <td className="px-6 py-16 text-center text-lg text-slate-400" colSpan="6">
