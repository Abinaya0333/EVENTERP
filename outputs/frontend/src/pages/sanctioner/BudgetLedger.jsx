...
function eventTitle(budget) {
  return budget.event_title || budget.event?.title || `Budget #${budget.id}`;
}
...
export default function BudgetLedger() {
  ...
  const approved = useMemo(
    () => budgets.filter((budget) => String(budget.status || '').toUpperCase() === 'APPROVED'),
    [budgets],
  );
  ...
  const totalApproved = approved.reduce(
    (sum, budget) => sum + Number(budget.approved_amount || budget.total_amount || 0),
    0,
  );
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            <div className="rounded-xl border border-slate-100 bg-white p-8 shadow-sm">
              <div className="border-l-8 border-blue-500 pl-6">
                <p className="text-3xl font-bold">{money(totalApproved)}</p>
                <p className="text-slate-500">Total Approved</p>
              </div>
            </div>
            <div className="rounded-xl border border-slate-100 bg-white p-8 shadow-sm">
            ...
            <div className="rounded-xl border border-slate-100 bg-white p-8 shadow-sm">
              <div className="border-l-8 border-emerald-500 pl-6">
                <p className="text-3xl font-bold">{money(remaining)}</p>
              ...
              <tbody className="divide-y divide-slate-100">
                {approved.length ? (
                  approved.map((budget) => {
                    ...
                    return (
                      <tr key={budget.id}>
                        <td className="px-6 py-5 font-semibold">{eventTitle(budget)}</td>
                  ...
                  <tr>
                    <td className="px-6 py-16 text-center text-lg text-slate-400" colSpan="5">
                      No approved events with budgets yet
                    </td>
                  </tr>
                )}
              </tbody>
